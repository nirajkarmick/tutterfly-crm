...
export class FormManagementComponent implements OnInit {
  ...
  fieldTypeArray = [
    ...
    ['address', 'assets/img/field_icons/Text-Area.png'],
    ['select', 'assets/img/field_icons/dropdown.png'],
    ['search_account', 'assets/img/field_icons/Text-Field.png'],
    ['search_contact', 'assets/img/field_icons/Text-Field.png'],
    ['search_per_account', 'assets/img/field_icons/Text-Field.png'],
  ...
  constructor(private router: Router,
    ...
    this.form_type = this.route.snapshot.queryParams['type'];
    this.cuurent_category=this.form_type; 
  ...
  onDropSt(dropResult) {
    ...
    this.standardFields = applyDrag(this.standardFields, dropResult);
  }
  hasNoChildhasNoChild(evnt){
...
colSorted(evnt){   
  ...
  choose_picklist(){
    ...
    if(this.picklist_selected){
      ...
      this.formService.load_picklist(this.picklist_selected).subscribe((data: any) => {
          console.log(data); console.log(this.picklist_selected); 
    ...
    if(this.isStandard){
      this.formService.getAllstandard_admin(this.cuurent_category).subscribe((data: any) => {
        ...
        if(data && data.standard_fields!=undefined){
         ...
         setTimeout(() => {    
        $('.remove_tbl').remove();
        $('.sortable-header').after('<table style="width: 100%;" class="remove_tbl"><tbody><tr><th>Name</th><th>Mandatory</th><th>Show/Hide</th><th>Edit</th></tr></tbody></table>')
     }, 1400); 
      });
  ...
  active_inactive_add_field(id,val){
    this.formService.update_add_fields_status(id,val,this.cuurent_category).subscribe((data: any) => {
  ...
  set_mandotaory_add_field(id,val){
    this.formService.set_mandotaory_add_field(id,val,this.cuurent_category).subscribe((data: any) => {
