...
export class AccountsGridComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    private ngZone: NgZone, private accountServiceService: AccountServiceService,
    ...
    this.accountServiceService.setUpAccountsMap().subscribe(data => {
      ...
      this.accountServiceService.getAllAccounts().subscribe((data: any) => {
        ...
        if (data.account_views != undefined) {
          for (let e of data.account_views) {
    ...
    if (viewId != 0 && viewId != undefined) {
      ...
      this.accountServiceService.setUpAccountsMap().subscribe(data => {
        ...
        this.accountServiceService.fetchView(viewId, 0).subscribe((datas: any) => {
          this.filterArray = datas.account_filters;
...
defaultPicklistArray(pickObject){
  ...
  delete_view(viewId) {
    ...
    if (conf) {
      this.formService.delete_view(viewId, 'account').subscribe((data: any) => {
        console.log(data);
        this.popUpMsg = JSON.stringify(data);
        this.openDialog();
        this.refreshAccountsGrid();
